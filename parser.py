...
class Entity:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """
        Преобразует объект Entity в словарь
        ...
        # Для списка secondary_oked используем исходный список (asdict уже преобразовал его)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Entity":
        ...
        """
        # Обрабатываем специальные случаи преобразования
...
class SQLiteSaver:
    ...
    def all(self) -> List[Entity]:
        with sqlite3.connect(self.db_name) as conn:
            ...
            # Convert each row to a dictionary before passing to Entity.from_dict
            entities = []
            for row in rows:
                row_dict = dict(zip(column_names, row))
                entities.append(Entity.from_dict(row_dict))
